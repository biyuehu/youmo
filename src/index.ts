...
;(async () => {
  ...
  try {
    ...
    if (tiebaList.length === 0) {
      ...
      console.log('[DEBUG] 贴吧列表原始数据:')
      console.log(JSON.stringify(tiebaList, null, 2))
      console.log('[DEBUG] 贴吧名称列表:')
      tiebaList.forEach((tieba, idx) => {
        console.log(`  ${idx + 1}. ${tieba.forum_name} (is_sign=${tieba.is_sign}, level=${tieba.level_id})`)
      })
    }
    ...
    for (let i = 0; i < tiebaList.length; i += batchSize) {
      ...
      for (let j = 0; j < batchTiebas.length; j++) {
        ...
        const signPromise = (async () => {
          try {
            // TODO: 暂时注释掉真实签到接口，用于调试
            console.log(`[DEBUG] 模拟签到: ${tiebaName} (index=${tiebaIndex}, is_sign=${tieba.is_sign})`)
      ...
      // 每批次后输出简洁的进度统计
      console.log(
        `✅ 批次${currentBatch}完成: ${i + batchTiebas.length}/${tiebaList.length} | ` +
          ...
          for (const failedTieba of failedTiebas) {
            ...
            const retryPromise = (async () => {
              try {
                ...
                // TODO: 暂时注释掉真实签到接口，用于调试
                console.log(`[DEBUG] 模拟重试签到: ${tiebaName}`)
                // 更新计数
                failedCount--
                alreadySignedCount++
                console.log(`✅ ${maskTiebaName(tiebaName)} 第${retryCount}次重试签到成功(模拟)`)
                ...
                console.log(
                  `❌ ${maskTiebaName(tiebaName)} 第${retryCount}次重试签到仍然失败: ${processedResult.message}`
                )
